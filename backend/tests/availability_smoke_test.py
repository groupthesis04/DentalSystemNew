...
def cleanup(emails: set[str], availability_ids: set[str] | None = None) -> None:
    store = MySQLStore()
    store.initialize()
    data = store.load()
    user_ids = {
    ...
    availability_ids = availability_ids or set()
    data["availability"] = [
        item for item in data["availability"] if item.get("id") not in availability_ids
    ]
    store.save(data)
...
def main() -> None:
    ...
    doctor_email = os.environ.get("DRMS_TEST_DOCTOR_EMAIL", "doctor@smilecare.local")
    doctor_password = os.environ.get("DRMS_TEST_DOCTOR_PASSWORD", "")
    if not doctor_password:
        raise RuntimeError("Set DRMS_TEST_DOCTOR_PASSWORD before running this test.")
    doctor_name = ""
    ...
    try:
        ...
        doctor_name = logged_in["user"]["name"]
        ...
        availability_ids.update(item["id"] for item in batch.get("availability_created", []))
        ...
        cleanup({email, competing_email}, availability_ids)
