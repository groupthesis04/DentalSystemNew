...
FRONTEND_URL = os.environ.get("DRMS_FRONTEND_URL", "http://127.0.0.1:5173").rstrip("/")
...
class DentalRequestHandler(
    ...
    # Security headers added to every HTML, asset, and API response.
    ...
    # Standard HTTP methods. Feature-specific work is delegated to routes/.
    def do_GET(self) -> None:
        ...
        if parsed.path.startswith("/api/"):
            ...
            return
        self.redirect_to_frontend(parsed)
    ...
    # Shared API protection: request limits and same-origin checks.
    ...
    # Vue is served by Vite during development; this server handles only the API.
    def redirect_to_frontend(self, parsed) -> None:
        route = parsed.path if parsed.path.startswith("/") else "/"
        location = f"{FRONTEND_URL}{route}"
        if parsed.query:
    ...
    # JSON request and response helpers used by every feature route.
    ...
    # Login sessions, CSRF verification, and role checks.
    ...
    # Map each API URL to one clearly named feature method.
...
def main() -> None:
    ...
    print(
        f"Serving Dental Record Management API at http://{args.host}:{args.port} "
        f"using {args.storage.upper()} storage. Vue frontend: {FRONTEND_URL}"
    )
    try:
